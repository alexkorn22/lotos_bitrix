				...
				foreach ($arParams["PROP_VALUE"] as $ipr => $PROP_ID) {
					...
					while($ob = $res->GetNextElement()){
						...
						if(!empty($arResult["PRODUCT_PRICE_ALLOW"]) && !empty($arPriceCodes) || empty($arParams["PRODUCT_PRICE_CODE"]))
							$arFields["PRICE"] = CCatalogProduct::GetOptimalPrice($arFields["ID"], 1, $USER->GetUserGroupArray(), "N", $arPriceCodes);
						...
						if(!empty($arFields["SKU_PRODUCT"]) && !empty($arResult["PROPERTIES"])){
							...
							foreach ($arFields["SKU_PRODUCT"] as $ir => $arOffer) {
								...
								if($active){
									...
									if(!empty($arResult["PRODUCT_PRICE_ALLOW"]) && !empty($arPriceCodes) || empty($arParams["PRODUCT_PRICE_CODE"]))
										$arFields["PRICE"]   = CCatalogProduct::GetOptimalPrice($arOffer["ID"], 1, $USER->GetUserGroupArray(), "N", $arPriceCodes);
						...
						//price count
						$arPriceFilter = array("PRODUCT_ID" => $arFields["ID"], "CAN_ACCESS" => "Y");
